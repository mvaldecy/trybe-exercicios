...
function averageAge() {
  ...
  const ages = books.map((aux) => aux.releaseYear - aux.author.birthYear)
  const sumAges = ages.reduce((acc, curr) => acc + curr)
  return sumAges / n
...
const expectedResult3 = {
  ...
  function longestNamedBook() {
    // escreva seu código aqui
    return books.reduce((curr, acc) => (curr.name.length > acc.name.length) ? curr : acc)
  }
